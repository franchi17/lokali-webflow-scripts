...
(function () {
  ...
  var TS_KEYS = {
    created_at: 1, updated_at: 1, vendor_reply_at: 1, deactivated_at: 1, published_at: 1,
  ...
  var vendors = {
    ...
    updateMe: function (payload) {
      ...
      var fields = {
        ...
        // #147/SEC-050: the address geo passthrough was REMOVED 2026-08-24 —
        // the six geo columns are written server-side by /address/resolve
        // (service role) and are REVOKED from `authenticated`, so naming any
        // of them here would fail the WHOLE profile PATCH with 42501. Do not
        // re-add them (the SEC-024 rebuild-drift lesson, same shape).
    ...
    // Chain per vendor: first gallery photo -> first service MAIN image ->
    // first product MAIN image -> extra service photo -> extra product photo.
    // (services/products.image_url is the listing's main image and carries its
    // own #149 focal point; service_photos/product_photos are only the EXTRA
    // shots — the first ship queried only those and missed every vendor whose
