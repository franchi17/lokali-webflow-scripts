/*
 * lokali-qr-kit.js — QR encoder + renderers for the vendor dashboard (#163).
 *
 * Exposes window.LokaliQR:
 *   make(text)                 -> { count, isDark(row,col) }
 *   toSvg(text, {px, dark})    -> standalone <svg> markup (crisp at any size)
 *   toPngDataUrl(text, px)     -> data: URL PNG (for the Download button)
 *
 * Loaded page-level on /vendor-dashboard/marketing ONLY (pinned @1.4.N tag,
 * same ship process as every other script). lokali-marketing.js renders its
 ...
 * QR spec minimum — do not shrink it, phone cameras need it).
 *
 * The encoder below is qrcode-generator v2.0.4 by Kazuhiko Arase, MIT,
 * vendored VERBATIM (round-trip verified against the jsQR decoder before
 * shipping). https://github.com/kazuhikoarase/qrcode-generator
 *
 * Two mirrored copies (repo rule — edit BOTH, byte-identical):
 *   scripts/lokali-qr-kit.js
 *   lokali-webflow-scripts/scripts/lokali-qr-kit.js
 */
(function () {
  'use strict';
