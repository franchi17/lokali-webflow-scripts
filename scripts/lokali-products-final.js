...
const LokaliProductsPage = (() => {
  ...
  const applyFocusPreview = () => {
    ...
    const hint = document.getElementById('lok-product-focus-hint');
  ...
  const initFocusDrag = (thumb) => {
    ...
    if (!document.getElementById('lok-product-focus-hint')) {
      const hint = document.createElement('div');
      hint.id = 'lok-product-focus-hint';
  ...
  const wireCoverFocusDrag = (imEl, persistNow) => {
    ...
    const done = async () => {
      ...
      try { await window.LokaliAPI.products.update(editingId, payload); } catch (e) {}
    };
  ...
  const renderCoverPreview = (body, coverUrl, persistNow) => {
    ...
    im.id = 'lok-product-cover-preview';
  ...
  let _lcOrg = null;
  const lcPhotoCap = () => { const m = /(\d+) of (\d+) photos/.exec(document.getElementById('lok-product-gallery')?.textContent || ''); return m ? parseInt(m[2], 10) : 0; };
  ...
  const lcMountPreview = () => {
    ...
    if (!_lcOrg) _lcOrg = LUI.organize(fv, { kind: 'product', ids: { img: 'product-img-input', name: 'product-name', desc: 'product-description', price: 'product-price', quote: 'product-quote-based', active: 'product-is-active', ship: 'product-shipping', pickup: 'product-pickup-only', delivery: 'product-delivery', stock: 'product-stock', turnaround: 'product-turnaround' }, hosts: { gallery: 'lok-product-gallery', video: 'lok-product-video', buy: 'lok-product-buy' } });
  ...
  const populateForm = (product) => {
    ...
    if (product.image_url) {
      ...
      _imgFocus = (product.image_focus_x != null && product.image_focus_y != null)
        ? { x: product.image_focus_x, y: product.image_focus_y } : null;
      applyFocusPreview();
    }
  ...
  const renderSubcatPills = () => {
    const sel = document.querySelector('#lok-product-subcat [data-subcat-select]');
    ...
    try {
      ...
      if (d.ok) {
        ...
        setSubcatHint(editingId
          ? 'Thanks! Once approved, this product gets the tag automatically.'
          : 'Thanks! Save this product and it gets the tag automatically once approved.', 'ok');
